...
def makePlot( var, nbins, xmin, xmax, 
              ...
              antiMuRlxTauIsoForQCDYield=False,
              subtractBGForQCDShape=False, embedForSS=False, relSelection={}, osForWExtrapolation=True, incQCDYield=99999., qcdYieldInclusiveExtrapolation=False, isOneJet=False, dataComps={}, cutName='', isZeroB=False):
    ...
    if replaceW:
        ...
        if shift and 'WShape' in shift:
            print '>>>>>>>>>>>>>>>>>> Shifting W for tau fake rate:'
            if 'Up' in shift:
    ...
    if qcdYieldInclusiveExtrapolation:
        ...
        cat_qcd_cut = sscut
        if antiMuIsoForQCD:
            print 'INFO: Inverting mu iso for QCD yield extrapolation'
        ...
        if antiMuRlxTauIsoForQCDYield:
            print 'INFO: Inverting mu and relaxing tau iso for QCD yield extrapolation'
    ...
    # Scale QCD yield for mtt < 60 by factor 1.1 in one-jet categories unless tau iso is relaxed
    if (isOneJet or isZeroB) and not antiMuRlxTauIsoForQCD:
    ...
    if cat_J1B in cut:
        ...
        print 'Old yield', scaleTT, 'new yield', scaleTTNew, 'scale', scaleTTNew/scaleTT
        print 'Ints', osQCD.Hist('TTJets').obj.Integral(), osQCD.Hist('TTJets').weighted.Integral()
        osQCD.Hist('TTJets').Scale(scaleTTNew/scaleTT)
        print 'Ints', osQCD.Hist('TTJets').obj.Integral(), osQCD.Hist('TTJets').weighted.Integral()
        print 'New yield', osQCD.Hist('TTJets').Integral()

    # osQCD.Group('electroweak', ['WJets', 'Ztt_ZL', 'Ztt_ZJ','VV'])
    if not embed:
        osQCD.Group('Ztt', ['Ztt', 'Ztt_TL'])
...
if __name__ == '__main__':
    ...
    isTwoJet = cutstring.find('Xcat_J2B0X')!=-1
    ...
    # QCD handling
    antiMuIsoForQCD = isOneJet or cutstring.find('Xcat_VBF_looseX')!=-1 or cutstring.find('Xcat_VBFX')!=-1 or cutstring.find('Xcat_J0_highX')!=-1 or isBtag or cutstring.find('Xcat_J2B0X')!=-1
    antiMuRlxTauIsoForQCD = False # cutstring.find('Xcat_J1_high_mediumhiggsX')!=-1 or cutstring.find('Xcat_VBF_tightX')!=-1 or cutstring.find('Xcat_J1_high_lowhiggsX')!=-1 or cutstring.find('Xcat_J1_mediumX')!=-1 

    antiMuRlxTauIsoForQCDYield = False #cutstring.find('Xcat_J1_high_mediumhiggsX')!=-1 or cutstring.find('Xcat_VBF_tightX')!=-1 

    qcdYieldInclusiveExtrapolation = isVBF# or cutstring.find('Xcat_J1_high_mediumhiggsX')!=-1 

    subtractBGForQCDShape = not antiMuIsoForQCD and not antiMuRlxTauIsoForQCD
    ...
    # > python -i plot_H2TauTauDataMC_TauMu_All.py /data/steggema/Sep19MuTau/ tauMu_2012_cfg.py -C 'Xcat_IncX && mt<30' -H svfitMass -b
    # > print osQCD
    if isVBF:
        print 'WARNING, taking QCD yield from external calculation, Sep19MuTau'
    # incQCDYield = 18709.8 #Aug07 with pt>20
    incQCDYield = 4481.0 # Sep19 with pt>30
    ...
    parameters = {'cut':options.cut, 'anaDir':anaDir, 'selComps':selComps, 'weights':weights, 
      ...
      'antiMuRlxTauIsoForQCDYield':antiMuRlxTauIsoForQCDYield,
      ...
      'incQCDYield':incQCDYield, 'qcdYieldInclusiveExtrapolation':qcdYieldInclusiveExtrapolation, 'isOneJet':isOneJet, 'dataComps':dataComps, 'cutName':options.cutName, 'isZeroB':isZeroB} #'blind':options.blind, 

    if options.allPlots:
        drawAll(plots_All, 'PlotsSep04/', options.blind, parameters, isMSSM)
    else:
        if makeQCDIsoPlots:
