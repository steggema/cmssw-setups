...
from plot_H2TauTauDataMC_TauMu_Inclusive import makePlot as makePlotInclusive
from plot_H2TauTauDataMC_TauMu_Inclusive import handleW as handleW
...
def makePlot( var, weights,
    ...
    # oscut = str(inc_sig & Cut('l1_charge*l2_charge<0 && mt<30') & cat_VBF)
    print '[OS]', oscut
...
if __name__ == '__main__':
    ...
    cutW = options.cut
    cutW = cutW.replace('Xcat_VBF_looseX', cut_VBF_Rel_W)
    cutW = cutW.replace('Xcat_VBF_tightX', cut_VBF_Rel_W)
    cutW = cutW.replace('Xcat_VBFX', cut_VBF_Rel_W)
    cutW = cutW.replace('diTau_charge==0', '1.')

    print 'CUT W', cutW

    inc_fwss, inc_fwos, inc_w_mt_ratio_ss, inc_w_mt_ratio_os, inc_w_mt_ratio = handleW(
        anaDir, selComps, weights,
        cutW, weight, options.embed, cfg.VVgroup, nbins=30, highMTMin=60., highMTMax=120.
        )
